...
const Item = ({ verseData }) => (
  <ScriptureCard style={styles.scriptureCard} data={verseData} />
);

const TopicResultsList = (props) => {
  console.log("WAIT");
  const navigation = useNavigation();
  const item = props.topicSelected;
  ...
  useEffect(() => {
    const getData = async () => {
      const apiResponse = await fetch(
        "https://bibleseek-3ade5-default-rtdb.firebaseio.com/topics/" + item + ".json"
      );      
